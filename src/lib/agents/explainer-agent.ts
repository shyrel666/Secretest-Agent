...
function parseLearningLessonDocument(rawContent: string): LessonDocument {
  ...
  for (const candidate of collectJsonPayloadCandidates(rawContent)) {
    try {
      ...
      const parsed = learningLessonDocumentSchema.parse(payload);

      return {
        contentMarkdown: stripPracticeSection(parsed.contentMarkdown),
        practiceQuestions: parsed.practiceQuestions.map(normalizePracticeQuestion),
      };
    } catch (error) {
      lastJsonError = error;
    }
...
请提供完整的讲解内容。`,
  ...
  private async invokeStructuredLearningLesson(messages: LearningLessonMessage[]) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        return {
          ...
          lessonDocument: parseLearningLessonDocument(responseContent),
        };
      } catch (error) {
        if (!responseContent) {
          lastError = error;
